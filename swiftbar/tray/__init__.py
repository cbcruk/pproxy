...
sibling plugin script ``pproxy.5s.py`` shells out to *this* package to do
the work — start/stop the Node proxy, toggle the macOS system proxy, and
open the rules file in an editor. Everything here is plain Python (no
third-party deps) so the plugin runs under a stock interpreter.

Modules:
    - :mod:`tray.daemon` — start/stop the proxy as a detached process
    - :mod:`tray.sysproxy` — point the macOS system proxy at it
    - :mod:`tray.config` — the editor and proxy-command settings
