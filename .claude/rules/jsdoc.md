...
## Applying these rules in pproxy

- The package entry point is `src/index.ts`; examples import from `pproxy`.
  Internal imports carry the `.js` extension, as NodeNext requires.
- `src/index.ts` carries the package's `@module` comment. The package exposes a
  single entry, so no other file needs one; the file-overview comment at the top
  of each module is plain prose without the tag.
...
- `{@link}` is the tag this repo uses for cross-references, and `@throws If ...`
  is written as prose: everything here throws a plain `Error`, so there is no
  type worth putting in braces.
- Say which side of the boundary a type sits on. Rules-file types
  (`RuleData`, `GraphQLConditionData`) keep snake_case field names so one
  `rules.json` drives both this implementation and the archived Python one;
  the in-memory types are camelCase. A reader has to know which they are
  holding.
- The engine knows nothing about mockttp, and that is load-bearing — it is what
  makes rules unit-testable without a proxy. Keep it visible in the docs for
  `RuleEngine`, `ProxyRequest`, and the adapter in `proxy.ts`.
- Module-local helpers and constants carry a comment too, not just the exported
  surface: they hold the non-obvious details (fnmatch semantics, the mockttp
  listen patch, why a match is carried across two callbacks).
- Verify with `npm run typecheck` and `npm test` in the same change.
